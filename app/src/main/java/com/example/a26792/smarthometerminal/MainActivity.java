...
// Register the BroadcastReceiver
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                for (BluetoothDevice device : pairedDevices) {
                    if ((position--) == 0) {
                        ...
                        ConnectThread connectThread = new ConnectThread(device, mBluetoothAdapter);
                        connectThread.start();
                    }
        ...
        //搜索到的其他设备列表的点击事件监听
        mBluetoothDevicesList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Toast.makeText(MainActivity.this, "connecting" + position, Toast.LENGTH_SHORT).show();
                ConnectThread connectThread = new ConnectThread(bluetoothDeviceList.get(position), mBluetoothAdapter);
                connectThread.start();
